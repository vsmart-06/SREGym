...
def _session_file(run_dir: Path) -> Path:
    return run_dir / "copilot-cli.jsonl"


def test_no_jsonl_file_returns_none(tmp_path):
    assert copilot.convert_file(_session_file(_run_dir(tmp_path))) is None


def test_empty_file_returns_none(tmp_path):
    run_dir = _run_dir(tmp_path)
    (run_dir / "copilot-cli.jsonl").write_text("")
    assert copilot.convert_file(_session_file(run_dir)) is None
...
def test_non_json_lines_are_skipped_not_fatal(tmp_path):
    ...
    t = copilot.convert_file(_session_file(run_dir))
